...
<div class="flex flex-col w-screen h-screen items-center mt-20 justify-center p-5 m-0 h-screen mt-5 w-full"></div>
	...
	<div class="mt-10 w-96 h-full flex flex-col overflow-auto space-y-2 p-2.5 rounded border border-gray-300">
		{#each logs as log}
			{#if log.author === Author.User}
				<div class="bg-blue-500 text-white p-2 rounded ml-auto">{log.message}</div>
			{:else}
				<div class="bg-green-500 text-white p-2 rounded mr-auto w-[50%] h-[fit-content]">{log.message}</div>
			{/if}
		{/each}
	</div>
</div>
