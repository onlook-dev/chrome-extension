...
const sendColorMode = () => {
	platform.tabs.query({ active: true, currentWindow: true }, ([tab]: chrome.tabs.Tab[]) => {
		tab &&
...
export const getColorMode = () => {
	platform.storage.sync.get([storagekey], (value: any) => {
		let found_value = value[storagekey]

		const is_default = found_value ? value[storagekey] === defaultcolormode : false
...
platform.contextMenus.onClicked.addListener(
	({ parentMenuItemId, menuItemId }: any, tab: chrome.tabs.Tab) => {
